...
## Project Description
RecoverRight is a web app that is a daily recovery check-in platform designed for MLB players, physicians, and coaches. It is built using React with shadcn UI components and TailwindCSS for styling on the frontend, a Next.js backend, and Supabase for authentication, user profile storage, and data persistence. The landing page has sign-up and sign-in options where users select their role (player, physician, or coach). During sign-up, users must enter their name and age, which will be stored in their profile. Once signed in, players are able to submit a daily reflection describing how their body feels; this reflection is sent to Gemini AI, which returns a personalized recovery plan including mobility work, hydration guidance, and sleep recommendations. Once their dashboard a player will see their dashboard displaying all past recordings; selecting a recording opens a detail page showing the reflection, AI insight, and associated metadata. The dashboard also provides follow-up prompts referencing previous logs, visual progress graphs, streak tracking, and motivational feedback. Physicians have a dashboard listing all registered players, where they can view individual logs and send private messages to each player. Coaches also have a dashboard displaying all players with a color-coded health indicator ranging from red (high soreness or fatigue) to green (fully healthy), along with a quick summary of player condition. The app allows for seamless communication between a player, physician, and coach so that everyone is always on the same page while keeping all health related information in one location. 
